...
from .schemas import (
    ...
    FileOut, ProjectStructureOut, MessageResponse,
)
...
def get_project_structure(request, project_id: int):
    """Returns the directory tree visible to the requesting user."""
...
def check_file_conflict(request, project_id: int, data: ConflictCheckInput):
    ...
    conflict = check_conflict(project, data.directory_type, data.date_directory, stored_name)
...
def upload_file(
    ...
    mention_user_ids: Optional[str] = Form(None),  # "1,2,3"
):
    project = get_accessible_project(request.auth, project_id)
    check_directory_access(request.auth, project_id, directory_type)

    if directory_type in ("date", "edit") and not date_directory:
        raise HttpError(400, "date_directory (YYYY-MM-DD) is required for date and edit types")

    stored_name = make_stored_name(display_name)

    user_ids = []
    if mention_user_ids:
        try:user_ids = [int(i.strip()) for i in mention_user_ids.split(",") if i.strip()]
        except ValueError:
